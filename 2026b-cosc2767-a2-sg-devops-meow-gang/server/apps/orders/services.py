...
# Acknowledgement: Supplied RMIT Store application; Django and Python Decimal documentation; OpenAI Codex used for test-design guidance.
...
# Keep pricing calculations separate for unit tests
def subtotal_for(lines):
    """Calculate the subtotal from unit-price and quantity pairs."""
    line_totals = (
        money(Decimal(unit_price) * int(quantity))
        for unit_price, quantity in lines
    )
    return money(sum(line_totals, ZERO))

def total_for(subtotal, total_tax):
    """Calculate the final payable total."""
    return money(Decimal(subtotal) + Decimal(total_tax))
...
def create_order(user, items, payment, request=None):
    ...
    for product_id, quantity in wanted.items():
        ...
        subtotal = subtotal_for(
            (item.purchase_price, item.quantity) for item in order_items
        )
        total = total_for(subtotal, total_tax)
